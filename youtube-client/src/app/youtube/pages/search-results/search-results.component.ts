...
export class SearchResultsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.store.pipe(takeUntil(this.unsubscribe$)).subscribe((state) => {
      this.searchResponse = state.searchReducer.search
        ? state.searchReducer.search[state.searchReducer.search.length - 1]!
        : undefined;
      this.createdCards = state.videoCardsReducer.cards?.length
        ? state.videoCardsReducer.cards
        : [];
    });
  }
